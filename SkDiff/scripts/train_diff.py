...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/train/train_diff.yml")
    ...
    val_iter = inf_iterator(val_loader)
    ...
    null_ids = null_inputs["input_ids"].to(args.device)
    null_attention_mask = null_inputs["attention_mask"].to(args.device)
    ...
    def validate(it_idx: int):
        ...
        for _ in range(len(val_loader.dataset)):
            batch = next(val_iter).to(args.device)
            context = batch.get("clip_emb", None)
            with torch.no_grad():
                if context is None:
                    text_inputs = tokenizer(batch.caption, return_tensors="pt", padding='max_length', truncation=True)
