...
def main(args):
    ...
    # --- 【新增】初始化计数器 ---
    margin_met_count = 0
    total_items = len(item_ids_this_process)
    ...
    for item_id in progress_bar:
        ...
        if os.path.exists(win_dir) and os.path.exists(lose_dir):
            # 如果跳过，也应该计入总数以便统计
            # 如果需要对已存在的是否符合margin进行统计，需要额外逻辑
            # 这里我们假设跳过的不计入符合数
            continue
        ...
        for attempt in range(max_attempts):
            ...
            progress_bar.set_postfix_str(f"ID: {item_id}, Try: {attempt+1}/{max_attempts}, A: {score_a:.4f}, B: {score_b:.4f}, Margin: {abs(score_a - score_b):.4f}")

            if abs(score_a - score_b) >= args.margin or attempt == max_attempts - 1:
                break
        
        # --- 【新增】检查最终结果并计数 ---
        if abs(score_a - score_b) >= args.margin:
            margin_met_count += 1
    ...
    # --- 【新增】打印最终统计结果 ---
    print(f"\n--- 结果统计 ---")
    print(f"处理的总样本数: {total_items}")
    print(f"符合Margin (>{args.margin}) 的样本数: {margin_met_count}")
    if total_items > 0:
        percentage = (margin_met_count / total_items) * 100
        print(f"符合率: {percentage:.2f}%")

if __name__ == '__main__':
    parser = ArgumentParser(description="根据分数和Margin生成DPO数据对（单卡版）")
    ...
    parser.add_argument('--margin', type=float, default=0.1, help="Win/Lose对之间的最小SKA分数差距")
