...
class Info:
    ...
    def loadHierarchy_recur(self, node, lines, joint_pos):
        for li in lines:
            if li.split()[0] == 'hier' and li.split()[1] == node.name:
                ...
                ch_node.parent = node
                self.bones.append((li.split()[1],li.split()[2]))
    ...
    def get_part_dict(self):
        ...
        while this_level:
            next_level = []
            for node in this_level:
                part_dict[node.name] = node.part
                next_level += node.children
            this_level = next_level
        return part_dict
    def get_bones_idx(self):
        bones_idx = []
        joint_names = list(self.get_joint_dict().keys())
        for each in self.bones:
            bones_idx.append((joint_names.index(each[0]),joint_names.index(each[1])))
        return bones_idx
    ...
    def set_counter(self,start=0):
        self.counter = start
    def assign_joint_part(self,joint:TreeNode):
        if joint.name == self.root.name:
            self.set_counter(0)
            joint.part = 0
