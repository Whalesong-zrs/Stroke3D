...
def prepare_batch(target, background= 'black'):
    ...
    target['normal'] = target['normal'].cuda() if target['normal'] is not None else None
    target['mask'] = target['mask'].cuda() if target['mask'] is not None else None
...
def validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS):
    ...
    with torch.no_grad():
        ...
        if hasattr(geometry,'render_skd'):
            sk = geometry.render_skd(target['mvp'],target['resolution'])[0]
            if sk.shape[-1] == 4:
                sk = sk[...,:3]
            result_dict['sk'] = sk
            result_image = result_image/2 + sk/2
    ...
    for it, target in enumerate(dataloader_validate):
        ...
        for k in result_dict.keys():
            ...
            if k == 'shaded':
                ...
                util.save_image(mask_dir + '/' + ('val_%06d_%s.png' % (it, k)), np_img.squeeze(axis=-1))
...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it,if_normal, if_pretrain, scene_and_vertices ):
        ...
        if if_pretrain:        
            ...
            return self.geometry.tick_rec(glctx, target, self.light, self.material, it , if_normal, self.FLAGS.mode, self.if_flip_the_normal)
            # if self.FLAGS.mv_guidance:
            #     return self.geometry.tick_mv(glctx, target, self.light, self.material, it , if_normal, self.guidance, self.FLAGS.mode, self.if_flip_the_normal)
            # else:
    ...
    for it, target in enumerate(dataloader_train):
        ...
        if FLAGS.local_rank == 0:
            ...
            if  save_image:
                ...
                # if 'sk' in result_dict.keys():
                #     np_sk_img = result_dict['sk'].detach().cpu().numpy()
                #     util.save_image(FLAGS.out_dir + '/' + ('sk_%s_%06d.png' % (FLAGS.mode, img_cnt)), np_sk_img)
                img_cnt = img_cnt+1
            if save_video:
                with torch.no_grad():
                    ...
                    if FLAGS.mode =='geometry_modeling':
                        ...
                        # video_image = buffers['shaded'][0, ..., 0:3]
                    video_image = video.ready_image(video_image)
                    
        iter_start_time = time.time()
        if FLAGS.mode =='geometry_modeling':
            if it<=400 and not FLAGS.skeleton_init:
            # if it<0:
                if_pretrain = True
            else:
                if_pretrain = False
        ...
        with torch.cuda.amp.autocast(enabled= True):
            ...
            if if_pretrain == False:
                loss_dict = model(target, it, if_normal, if_pretrain= if_pretrain, scene_and_vertices =None)
                total_loss = 0
        ...
        if it % log_interval == 0 and FLAGS.local_rank == 0 and if_pretrain == False:
            sds_loss_avg = np.mean(np.asarray(sds_loss_vec[-log_interval:]))
            iter_dur_avg = np.mean(np.asarray(iter_dur_vec[-log_interval:]))
            
            remaining_time = (FLAGS.iter-it)*iter_dur_avg
            if optimize_geometry:
                print("iter=%5d, sds_loss=%.6f, mesh_lr=%.5f, time=%.1f ms, rem=%s, mat_lr=%.5f" % 
                    (it, sds_loss_avg, optimizer_mesh.param_groups[0]['lr'], iter_dur_avg*1000, util.time_to_text(remaining_time),optimizer.param_groups[0]['lr']))
            else:
                print("iter=%5d, sds_loss=%.6f, time=%.1f ms, rem=%s, mat_lr=%.5f" % 
                    (it, sds_loss_avg, iter_dur_avg*1000, util.time_to_text(remaining_time),optimizer.param_groups[0]['lr']))
...
class DatasetRec(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if self.repeat_idx is not None:
            ...
            try:
                nrm = io.imread(os.path.join(self.folder,f'nrm_{self.repeat_idx}_{idx}.png'))
            except:
                nrm = None
        else:
            img = io.imread(os.path.join(self.folder,f'tile_{idx}.png'))
        data['image'] = torch.tensor(img[...,:3],dtype=torch.float32)/255.0 # [0,255] -> [0,1]
        if nrm is not None:
            data['normal'] = torch.tensor(nrm[...,:3],dtype=torch.float32)/255.0*2-1 # [0,255] -> [-1,1] for normal
        if img.shape[-1] == 4:
            data['mask'] = torch.tensor(img[...,3],dtype=torch.float32)/255.0 # [0,255] -> [0,1]
        data['mv'] = self.cam_dict['mv'][idx]
        data['mvp'] = self.cam_dict['mvp'][idx]
        data['campos'] = self.cam_dict['campos'][idx]
        data['resolution'] = self.FLAGS.train_res
        data['spp'] = 1
        return data
    ...
    def collate(self, batch):
        ...
        return {
            'mv' : torch.stack(list([item['mv'] for item in batch]), dim=0),
            'mvp' : torch.stack(list([item['mvp'] for item in batch]), dim=0),
            'campos' : torch.stack(list([item['campos'] for item in batch]), dim=0),
            'resolution' : iter_res,
            'spp' : iter_spp,
            'image' : torch.stack(list([item['image'] for item in batch]), dim=0),
            'normal' : torch.stack(list([item['normal'] for item in batch]), dim=0) if 'normal' in batch[0] else None,
            'mask' : torch.stack(list([item['mask'] for item in batch]), dim=0) if 'mask' in batch[0] else None,
            'normal_rotate': None,
        }
...
if __name__ == "__main__":
    ...
    if FLAGS.config is not None:
        config_path = Path(FLAGS.config).expanduser().resolve()
        data = json.load(open(config_path, 'r'))
        for key in data:
            FLAGS.__dict__[key] = data[key]
        if FLAGS.envmap and not Path(FLAGS.envmap).is_absolute():
            FLAGS.envmap = str(config_path.parent.parent / FLAGS.envmap)
    ...
    # ==============================================================================================
    dataset_train    = DatasetRec(folder=FLAGS.data_dir,FLAGS=FLAGS)
    dataset_validate = DatasetRec(folder=FLAGS.data_dir,FLAGS=FLAGS, val=True)
    dataset_gif      = DatasetRec(folder=FLAGS.data_dir,FLAGS=FLAGS, val=True)
    ...
    base_mesh.material['bsdf'] = FLAGS.init_bsdf
    vertices = base_mesh.v_pos
    ...
    geometry, mat = optimize_mesh(glctx, geometry, mat, lgt, dataset_train, dataset_validate, FLAGS, 
                                    optimize_light=FLAGS.learn_light, optimize_geometry=not FLAGS.lock_pos, guidance= None)
    ...
    if FLAGS.random_textures:
        ...
        render.pad_uv(glctx, base_mesh, FLAGS.texture_res, 1)
    ...
    if FLAGS.local_rank == 0:
        ...
        obj.write_obj(FLAGS.out_dir, base_mesh)
