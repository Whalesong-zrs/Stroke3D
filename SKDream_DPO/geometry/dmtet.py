...
def _save(x: torch.Tensor,fname):
...
class DMTetGeometry(torch.nn.Module):
    ...
    def tick(self, glctx, target, lgt, opt_material, iteration, if_normal, guidance, mode, if_flip_the_normal):
        ...
        return {'sds_loss':sds_loss}
    
    def tick_rec(self, glctx, target, lgt, opt_material, iteration, if_normal, mode, if_flip_the_normal):
        buffers= self.render(glctx, target, lgt, opt_material, if_normal= if_normal, mode = mode, if_flip_the_normal = if_flip_the_normal)
        rendered_normal = buffers['shaded'][...,0:3] * buffers['shaded'][...,3].unsqueeze(-1).repeat(1,1,1,3) # black background
        rendered_mask = buffers['shaded'][...,3]
        ref_mask = target['mask']
        ref_normal = target['image']*ref_mask.unsqueeze(-1).repeat(1,1,1,3)
        
        img_loss = torch.nn.functional.mse_loss(rendered_normal, ref_normal) 
        mask_loss = torch.nn.functional.mse_loss(rendered_mask,ref_mask)
        return {'mask_loss':mask_loss,'img_loss':img_loss}