...
def get_custom_input_cameras(azimuths=[0,90,180,270], elevations=[20,20,20,20],radius=5.0):
    ...
    Ks = FOV_to_intrinsics(30).unsqueeze(0).repeat(len(azimuths), 1, 1).float().flatten(-2)
    extrinsics = c2ws[:, :12]
    
    intrinsics = torch.stack([Ks[:, 0], Ks[:, 4], Ks[:, 2], Ks[:, 5]], dim=-1)
    cameras = torch.cat([extrinsics, intrinsics], dim=-1)

    return cameras.unsqueeze(0).repeat(1, 1, 1)