...
from skdream.dataset import SKDreamDataset
...
def main(args):
    ...
    train_dataset = SKDreamDataset(root_dir=args.data_root_dir,p_simple_prompts=0,cond_channels=cond_channels,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                images = rearrange(batch["pixel_values"].to(dtype=weight_dtype),"b n c h w -> (b n) c h w")
                ...
                # Get camera embedding for conditioning
                controlnet_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)
                cameras = batch["cameras"].to(dtype=weight_dtype).reshape(batch_size*num_views,-1)
                ...
                model_pred = unet(
                    rearrange(noisy_latents, 'b n c h w -> (b n) c h w'),
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    camera_matrix_embeds=cam_mtx_emb.to(dtype=weight_dtype),
