...
class DLMesh(torch.nn.Module):
    ...
    def tick_rec(self, glctx, target, lgt, opt_material, iteration, if_normal, mode, if_flip_the_normal):
        buffers= self.render(glctx, target, lgt, opt_material, if_normal= if_normal, mode = mode, if_flip_the_normal = if_flip_the_normal)
        rendered_mask = buffers['kd'][...,3].unsqueeze(-1).repeat(1,1,1,3).to(torch.bool)
        rendered_normal = buffers['kd'][...,0:3]
        ref_normal = target['image']
        
        if self.FLAGS.rec_loss == 'mse':
            img_loss = torch.nn.functional.mse_loss(rendered_normal[rendered_mask], util.srgb_to_rgb(ref_normal)[rendered_mask])
            reg_loss = torch.nn.functional.mse_loss(opt_material['init_kd'].data, opt_material['kd'].data) * 10
        elif self.FLAGS.rec_loss == 'lpips':
            _loss_fn_vgg = lpips.LPIPS(net='vgg').cuda()
            def loss_fn_vgg(fun,x1,x2):
                ...
                # normalize to (-1,1)
                return fun(normalize_imagenet(x1),normalize_imagenet(x2)).mean()
            img_loss = loss_fn_vgg(_loss_fn_vgg,rendered_normal, util.srgb_to_rgb(ref_normal))
            # reg_loss = loss_fn_vgg(_loss_fn_vgg,opt_material['init_kd'].data, opt_material['kd'].data)
            reg_loss = torch.zeros_like(img_loss)
