# Modified from MVControl, adapted for multi-view conditions.
...
from skdream.unet import UNet2DConditionModel
from skdream.camera_proj import CameraMatrixEmbedding
from skdream.skdream import MultiViewControlNetModel
from skdream.scheduler import DDIMScheduler_ as DDIMScheduler
from skdream.utils.typing import *
...
@dataclass
class SKDreamPipelineOutput(BaseOutput):
    """
...
# diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_controlnet.StableDiffusionControlNetPipeline
class SKDreamPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        return SKDreamPipelineOutput(images=image)

    
    def _forward(
        self, 
        latent_model_input: Float[Tensor, "B N C H W"],
        controlnet_cond: Float[Tensor, "B N C H W"], #DIFF: multiview condition
        ...
        if c2ws is not None:
            cam_mtx_flattened = rearrange(c2ws, 'b n l -> (b n) l')
            cam_mtx_emb = self.camera_proj(cam_mtx_flattened)
        else:
            cam_mtx_emb = None
        ...
        # if False:
            ...
            if guess_mode and do_classifier_free_guidance:
                ...
                controlnet_sample = latent_model_input
                
            down_block_res_samples, mid_block_res_sample, temb_residual = self.controlnet(
                sample=controlnet_sample.to(self.controlnet.dtype),
                controlnet_cond=controlnet_cond.to(self.controlnet.dtype),
                timestep=t,
                encoder_hidden_states=controlnet_prompt_embeds.to(self.controlnet.dtype),
...
from skdream.attention import set_self_attn_processor, XFormersCrossViewAttnProcessor, CrossViewAttnProcessor

def load_skdream_pipeline(
